...
function addMarker(place)
{
    ...
    var image = "https://upload.wikimedia.org/wikipedia/commons/7/7e/Red_x.png";
    ...
    google.maps.event.addListener(marker, "click", function() { distance(marker, place.place_name + "," + place.admin_code1); });
...
//modified from code found at http://stackoverflow.com/questions/8404108/how-to-get-distance-between-a-marker-and-the-center-in-googlemaps
function distance(marker, query)
{
	var center =  new google.maps.LatLng(parseFloat(42.3770), parseFloat(-71.1256));
	var markerLatLng = marker.getPosition();
	var distance = google.maps.geometry.spherical.computeDistanceBetween(center, markerLatLng);
	distance = distance * 0.000621371;
	articles(marker, distance, query);	
}

/**
 * gets JSON of news articles
 */
function articles(marker, distance, query)
{
    ...
    .done(function(data, textStatus, jqXHR) {
        ...
        if (data.length > 0) {
            articleList = "<ul> The distance from Cambridge, Massachusetts is "+ distance +" miles away as the crow flies" ;
            ...
            articleList = "No news available. The distance from Cambridge, Massachusetts is "+ distance +" miles away as the crow flies";
        }
