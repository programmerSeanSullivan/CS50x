...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char lineHolder[needle - haystack +1];
            strncpy(lineHolders, haystack, needle - haystack);
            lineHolder[needle - haystack]='\0';
            
            char* requestLine[3] = {NULL, NULL, NULL};
            requestLine[METHOD]= strtok(lineHolder, " ");
            int x = 0;
            while(requestLine[x] != NULL && x < 2)
            {
                x++;
                requestLine[x] = strtok(NULL, " ");
            }
            ...
            if(requestLine[HTTP_VERSION]) == NULL || strcmp("HTTP/1.1",requestLine[HTTP_VERSION]))
            {
                error(505);
                continue;
            }
            
            char* qIndex = strchrnul(requestLine[REQUEST_TARGET], '?');
            char* eosIndex = strchr(requestLine[REQUEST_TARGET], '\0');
            // TODO: extract query from request-target
            char query[(eosIndex - qIndex < 1) ? 1 : eosIndex - qIndex] = "TODO";
            memcpy(
                query,
                (strncmp(qIndex, "\0", 1) == 0) ? qIndex : qIndex +1,
                (eosIndex - qIndex < 1) ? 1 : eosIndex - qIndex);
            absolutePath[qIndex - requestLine[REQUEST_TARGET]] = '\0';
            ...
            char path[strlen(root)+strlen(absolutePath)+1] = "TODO";
            strcpy(path, root);
